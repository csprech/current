...
const AI_NODE_DOCS: Record<NodeType, AINodeDoc> = {
  ...
  llmGenerate: { io: "text, image? → text", purpose: "LLM text generation for prompt expansion or image analysis; providers: Google, OpenAI, Anthropic, or Ollama (local, free, no API key)" },
  ...
  imageAction: { io: "image(s) → image", purpose: "Local image ops — rotate, flip, blur, adjust colors, Canny edge detect (ControlNet-style edge maps), aspect crop/pad, side-by-side, add text — free, no API cost" },
