      ...
      <div className="relative w-full h-full min-h-0 overflow-hidden rounded-lg">
        {nodeData.status === "loading" ? (
          ...
          <div className="group/text relative w-full h-full bg-neutral-900/40 p-2 overflow-auto nowheel">
            ...
            <div className="absolute top-1 right-1 flex gap-1 opacity-75 group-hover/text:opacity-100 focus-within:opacity-100 transition-opacity">
              <button
                onClick={handleCopyOutput}
                aria-label={copied ? "Copied to clipboard" : "Copy output to clipboard"}
                className={`current-media-action current-media-action--overlay nodrag nopan ${copied ? "!bg-[var(--current-success)]" : ""}`}
              ...
              <button
                ...
                className="current-media-action current-media-action--overlay nodrag nopan"
                title="Regenerate"
              >
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
